...
/**
 * Parse a fixed-format research proposal .docx buffer into form field values.
 * @param {Buffer} buffer
 * @returns {Promise<Record<string, unknown>>}
 */
async function parseResearchProposalDocx(buffer) {
  const zip = await JSZip.loadAsync(buffer);
  const documentXml = await zip.file('word/document.xml')?.async('string');
  if (!documentXml) {
    const err = new Error('Invalid DOCX file: missing document.xml');
    err.statusCode = 400;
    throw err;
  }
  ...
  if (!body) {
    const err = new Error('Invalid DOCX file: missing document body');
    err.statusCode = 400;
    throw err;
  }
  ...
  if (countFilledFields(parsed) === 0) {
    const err = new Error(
      'לא נמצא תוכן בטופס. ודאו שהעליתם קובץ Word מלא לפי הפורמט הקבוע.'
    );
    err.statusCode = 422;
    throw err;
  }
...
module.exports = { parseResearchProposalDocx };
